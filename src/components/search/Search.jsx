...
const gouvernorats = [ /* déjà défini, utile pour la navigation */ ];

function Search() {
  const navigate = useNavigate();

  const [country, setCountry] = useState("");
  const [region, setRegion] = useState(""); // Ville (gouvernorat)
  const [service, setService] = useState(""); // Secteur
  const [keyword, setKeyword] = useState(""); // Mot clé
  ...
  // Fetch gouvernorats (villes) au chargement
  useEffect(() => {
    fetch(`${API_BASE}/gouvernorats`)
      .then(res => res.json())
      .then(data => {
        setRegionOptions((data.governorats || []).map(g => ({ value: g.name, label: g.name })));
      })
      .catch(() => setRegionOptions([]));
  }, []);

  // Fetch secteurs (services) selon gouvernorat sélectionné
  useEffect(() => {
    if (!region) {
      setServiceOptions([]);
      setService("");
    ...
    fetch(`${API_BASE}/secteurs?gouvernorat=${encodeURIComponent(region)}`)
      .then(res => res.json())
      .then(data => {
        setServiceOptions((data.secteurs || []).map(s => ({ value: s.name, label: s.name, id: s._id })));
      })
      .catch(() => setServiceOptions([]));
  }, [region]);

  // Recherche utilisateurs selon filtres
  const handleSearch = async () => {
    // Prépare les paramètres pour le backend
    const params = {
      country,
      gouvernorat: region,
      secteur: service,
    ...
    // Save search params to localStorage for refresh recovery
    localStorage.setItem("felbled_searchParams", JSON.stringify(params));
    localStorage.setItem("felbled_searchUsers", JSON.stringify(data.users || []));
    // Navigation vers la page résultat avec les utilisateurs trouvés
